...
func post_agent(new_agent):
	var new_agent_json = new_agent.serialize()
	var new_agent_string = JSON.stringify(new_agent_json)
	var headers = ["Content-Type: application/json"]
	if post_agent_http_request.get_http_client_status() != HTTPClient.STATUS_CONNECTING:
		var error = post_agent_http_request.request(add_agent_endpoint_url, headers, HTTPClient.METHOD_POST, new_agent_string)
		if error != OK:
