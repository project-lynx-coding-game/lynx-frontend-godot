...
@onready var tile_object_counts_container: VBoxContainer = tile_data_container.get_node("ObjectCounts")

@onready var tile_position: Vector2i = Vector2i.ZERO
@onready var tile_type: String = ""
@onready var tile_object_counts: Dictionary = {}
...
func _wipe_tile_objects_container():
	for tile_object in tile_object_counts_container.get_children():
		tile_object_counts_container.remove_child(tile_object)
		tile_object.queue_free()
		
func _create_tile_object_label(tile_object):
	var tile_object_label = Label.new()
	tile_object_counts_container.add_child(tile_object_label)
	return tile_object_label
	
func _set_tile_object_label_text(tile_object, tile_object_label):
	var tile_object_count = str(tile_object_counts[tile_object])
	tile_object_label.text += tile_object
	if tile_object_count == "1":
		tile_object_label.text += " x" + tile_object_count
	
func _add_tile_objects_to_tile_objects_container():
	_wipe_tile_objects_container()
	for tile_object in tile_object_counts.keys():
